  ...
  return {
    ...
    async adapt(builder) {
      if (process.env.ENABLE_VC_BUILD) {
        ...
        if (edge || split) {
          throw new Error(
            "edge and split options can only be used with ENABLE_VC_BUILD"
          );
        }
...
async function v1(builder, external) {
  ...
  const prerendered_pages = Array.from(
...
async function v3(builder, external, edge, split) {
  ...
  async function generate_serverless_function(
    ...
    write(
      `${dirs.functions}/${name}.func/package.json`,
      JSON.stringify({ type: "commonjs" })
    );
